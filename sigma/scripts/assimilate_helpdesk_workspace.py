...
ERP_FILE = frappe.get_app_path('helpdesk', 'helpdesk', 'workspace', 'helpdesk', 'helpdesk.json')
WS_NAME = 'Helpdesk'
SIGMA_FILE = frappe.get_app_path('sigma', 'sigma', 'sigma', 'workspace', 'helpdesk', 'helpdesk.json')
...
def backup_workspace(ws_name):
    try:
        ...
        backup_dir = os.path.join(frappe.get_site_path(), 'public', 'files', 'workspace_backups', 'assimilate_helpdesk')
