...
class VisitorGuardAssignment(Document):
	...
	def validate_visitor(self):
		"""Ensure visitor exists (using Human Profile)"""
	...
	def set_visitor_name(self):
		...
		if self.visitor:
			...
			self.visitor_name = visitor.full_name
	...
	def notify_guard(self):
		...
		if guard_user:
			subject = f"Visitor Assignment: {visitor.full_name}"
			message = f"""
			<p>You have been assigned to escort a visitor:</p>
			<ul>
				<li>Visitor: {visitor.full_name}</li>
